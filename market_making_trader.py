...
class MarketMakingTrader(Trader):
    ...
    def __create_spread(self, order_book):
    ...
    def tick(self, market):
        ...
        (bid, ask) = self.__create_spread(order_book)
        # If we have net inventory
        if abs(self.current_position()) > 0:
            price_to_list = ask if self.current_position() > 0 else bid
            clearance_order = Order(ORDER_ID_GENERATOR, self.agent_id, price_to_list, "LIMIT", -1 * self.current_position())
