...
class Market:
    ...
    def submit_order(self, trader, order):
        ...
        if not self.__can_trader_have_multiple_active_quotes(trader):
        ...
        self.orderbook.add_order(order)
        trader.add_units_as_price(order.or)
    ...
    '''
    def __can_trader_have_multiple_active_quotes(self, trader):
        if type(trader).__name__ in ['MarketMakingTrader']
    ...
    def run():
        for tick in range(max_iterations):
            randomized_trader_order = random.shuffle(self.traders)
